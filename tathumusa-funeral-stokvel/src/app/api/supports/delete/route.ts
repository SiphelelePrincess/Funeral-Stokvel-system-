...
export async function POST(request: Request) {
  const body = (await request.json()) as { supportRequestId?: string };
  ...
  if (!body.supportRequestId) {
    return NextResponse.json(
      { ok: false, message: "Support request id is required." },
      { status: 400 },
    );
  }

  await callMutation(client, "supportRequests:remove", {
    supportRequestId: body.supportRequestId,
  });
  return NextResponse.json({ ok: true, message: "Support request removed." });
}
