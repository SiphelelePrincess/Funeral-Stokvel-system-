...
export async function POST(request: Request) {
  ...
  await callMutation(client, "claims:setStatus", { claimId: body.claimId, status: "approved" });

  return NextResponse.json({ ok: true, message: "Claim approved. Payment can now be processed." });
}
