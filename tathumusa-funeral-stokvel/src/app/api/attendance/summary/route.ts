...
export async function GET() {
  ...
  const items = await callQuery<unknown, Array<{ status: string }>>(
    client,
    "attendance:listByMember",
    { memberName: profile.name },
  );

  const total = items.length;
  const attended = items.filter((item) => item.status === "attended").length;

  return NextResponse.json({ ok: true, attended, total });
}
