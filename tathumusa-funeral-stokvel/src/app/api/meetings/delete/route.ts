...
export async function POST(request: Request) {
  ...
  if (!body.meetingId) {
    return NextResponse.json(
      { ok: false, message: "Meeting id is required." },
      { status: 400 },
    );
  }

  await callMutation(client, "meetings:remove", { meetingId: body.meetingId });
  return NextResponse.json({ ok: true, message: "Meeting removed." });
}
